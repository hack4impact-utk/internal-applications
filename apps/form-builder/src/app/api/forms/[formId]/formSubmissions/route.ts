...
import { createFormSubmission, getFormSubmissions } from "@/server/actions/formSubmissions";
import { NextRequest, NextResponse } from "next/server";
import { zCreateFormSubmissionRequest, zObjectId } from '@hack4impact-utk/internal-models';
...
export async function POST (
    ...
    if(!validationResult.success){return NextResponse.json({message: validationResult.error}, { status: 400 })}
    
    if(!zObjectId.safeParse(params.formId).success){return NextResponse.json({message: "Invalid ID"}, { status: 400 })}
    
    const result = await createFormSubmission(params.formId, requestBody); 
    if(!result){return NextResponse.json({message: "FormID not found"}, { status: 404 })}
